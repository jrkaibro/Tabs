...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="GxControl">
    <xsl:choose>
      <xsl:when test="@type = 'KnetTabs'">
        <xsl:call-template name="RenderKnetTabs"/>
      </xsl:when>
    </xsl:choose>
  </xsl:template>

  <!-- KnetTabs design render -->
  <!-- ///////////////////  Implement your render here  ///////////////////-->
  <xsl:template name="RenderKnetTabs">
    <span atomicselection="true">
      <xsl:call-template name="AddStyleAttribute"/>
      
	<div>      
	   <xsl:attribute name ="id">KnetTabs</xsl:attribute>
   	  <img>
